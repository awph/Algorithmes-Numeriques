...
Real100bits::Real100bits(const string& completeValueb10):type(Valid),S(1),M(0L),E(0),s(false)
{
	...
	else
	{
		...
		//Vérifie la présence de 0 seule afin de gérer le cas +-0
		bool isZero = true;
		...
		else
		{
			...
			//Calcul le décalage selon la position de la virgule
...
void Real100bits::computeSME()
{
	...
	{
		/*
		 Source : IEEE Std 754-1985 Revision of Reaffirmed 1990
...
Real100bits& Real100bits::operator+=(const Real100bits& r)
{
	...
	if(s != r.s)
	{
		bool ls = s;
		Real100bits r2(r);
		//On adapte le signe on conséquence, on fait toujours A-B dans la sous, A>B
		s = false;
		r2.s = false;
	...
	else
	{
		...
		while(a.E != b.E)
		{
			...
			if(!tabPlusOne(b.e, SIZE_E))
			{
				// Overflow !
				this->type = NaN;
				return *this; // TODO: Correct IEEE Format
			}
			b.computeSME();
		}

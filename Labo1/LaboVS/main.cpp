...
{
	cin.sync();
	cout << "Appuyer sur une <<Enter>> pour continuer !";
	cin.ignore();
}
...
int main(/*int argc,char* argv[]*/)
{
	...
	do 
	{
		...
		{
			cout << "Menu : " << endl
				<< "1) Convertir un nombre donnee" << endl
		...
		if(choix == 1)
		{
			...
			cin >> input;
			cout << endl << input << " avec notre interpretation : " << endl << endl;
		...
		{
			int n = 0;
			cout << "Entrez la valeure pour n : ";
			cin >> n;
			cout << "Pi vaut : " << endl;
			Real100bits r(approximationPI(n));
			r.printValue();
			cout << endl << "REMARQUE : A cause des doubles, on a l'impression que le nombre est le meme, mais les bits on bien change, donc pi est mieux approxime !" << endl;
			pause();
		}
		clear();
	}while(choix != 4);	
